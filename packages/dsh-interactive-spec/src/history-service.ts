...
import type { LiveSpecDoc, SpecTask, TaskState } from '@agentflow/live-spec-core';
 ...
 */
export function openDatabase(dbPath?: string): DatabaseSync | null {
 ...
 */
export function findNamespaceByCwd(db: DatabaseSync, cwd: string): NamespaceRecord | null {
  if (!cwd) return null;
  ...
  const namespaces: NamespaceRecord[] = rows.map((r) => ({
    id: String(r.id),
    name: String(r.name),
 ...
 */
function mapTaskState(dbState: string): TaskState {
  switch (dbState) {
    case 'done':
    case 'passed':
      return 'pass';
    ...
    case 'cancelled':
      return 'cancelled';
    default:
      return (dbState as TaskState) || 'pending';
  }
}
 ...
 */
export function listDagsByCwd(cwd: string, options?: HistoryServiceOptions): ListDagsResult {
  ...
  try {
    ...
    const rows = stmt.all(ns.id) as any[];

    const dags: DagSummary[] = rows.map((r) => ({
      id: String(r.id),
 ...
 */
export function getDagDetailByCwd(
  params: { cwd?: string; dag_id: string; namespace_id?: string },
  ...
  try {
    ...
    `);
    const taskRows = tasksStmt.all(dag_id, targetNsId || null, targetNsId || null) as any[];

    const dagMeta = safeJsonParse<Record<string, unknown>>(dagRow.metadata, {});
    const maxConcurrency =
      typeof dagMeta.max_concurrency === 'number'
