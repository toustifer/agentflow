...
import { listDagsByCwd, getDagDetailByCwd, findNamespaceByCwd, handleDagsRequest, handleDagRequest, apply, DAGS_API_PATH, DAG_API_PATH, } from '../src/index';
describe('Agentflow History API', () => {
    ...
    `);
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        insertNs.run('insighttutor', '知迹伴学', '2026-03-01T10:00:00Z', '2026-03-01T10:00:00Z', JSON.stringify({ workdir: 'D:\\myprogram\\InsightTutor' }));
        insertNs.run('other-project', 'Other Project', '2026-03-02T10:00:00Z', '2026-03-02T10:00:00Z', JSON.stringify({ workdir: 'D:/myprogram/Other' }));
        // Seed DAGs
        const insertDag = db.prepare(`
      INSERT INTO dags (id, namespace_id, title, status, metadata, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        insertDag.run('dag-invite-referral-growth', 'insighttutor', '全员推荐裂变：通用邀请码与返现激励', 'in_progress', JSON.stringify({ max_concurrency: 2 }), '2026-03-10T12:00:00Z', '2026-03-10T12:00:00Z');
        insertDag.run('dag-ai-tutor-dialogue', 'insighttutor', '智能对话风控与流式响应优化', 'completed', JSON.stringify({ max_concurrency: 3 }), '2026-03-05T08:00:00Z', '2026-03-05T08:00:00Z');
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        insertTask.run('ir-01', 'insighttutor', 'dag-invite-referral-growth', '数据表设计与邀请码生成服务', 'done', 'backend-dev', JSON.stringify([]), 2, '2026-03-10T12:01:00Z', '2026-03-10T12:01:00Z');
        insertTask.run('ir-02', 'insighttutor', 'dag-invite-referral-growth', '裂变邀请前端界面与海报生成', 'executing', 'frontend-dev', JSON.stringify(['ir-01']), 3, '2026-03-10T12:02:00Z', '2026-03-10T12:02:00Z');
        db.close();
    });
    afterEach(() => {
        try {
    ...
    describe('getDagDetailByCwd', () => {
        it('returns DAG details formatted as LiveSpecDoc', () => {
            const result = getDagDetailByCwd({ cwd: 'D:\\myprogram\\InsightTutor', dag_id: 'dag-invite-referral-growth' }, { dbPath: testDbPath });
            expect(result.ok).toBe(true);
            ...
            const t1 = spec.tasks.find((t) => t.task_id === 'ir-01' || t.id === 'ir-01');
            ...
            const t2 = spec.tasks.find((t) => t.task_id === 'ir-02' || t.id === 'ir-02');
            expect(t2.depends_on).toEqual(['ir-01']);
            expect(t2.state).toBe('executing');
        });
        it('gracefully returns error when dag_id is not found', () => {
            const result = getDagDetailByCwd({ cwd: 'D:\\myprogram\\InsightTutor', dag_id: 'non-existent-dag' }, { dbPath: testDbPath });
            expect(result.ok).toBe(false);
            expect(result.error).toContain('not found');
        });
        it('finds DAG by dag_id without providing cwd', () => {
            const result = getDagDetailByCwd({ dag_id: 'dag-invite-referral-growth' }, { dbPath: testDbPath });
            expect(result.ok).toBe(true);
        ...
        it('gracefully returns error when db is not found in getDagDetailByCwd', () => {
            const result = getDagDetailByCwd({ dag_id: 'dag-invite-referral-growth' }, { dbPath: path.join(tmpDir, 'missing.db') });
            expect(result.ok).toBe(false);
            expect(result.error).toBeDefined();
        });
    });
    describe('HTTP Route Handlers', () => {
        ...
        it('handleDagRequest handles valid cwd and dag_id queries', async () => {
            const req = new Request(`http://localhost${DAG_API_PATH}?cwd=${encodeURIComponent('D:\\myprogram\\InsightTutor')}&dag_id=dag-invite-referral-growth`);
        ...
        it('handleDagRequest returns 400 when dag_id is missing', async () => {
            const req = new Request(`http://localhost${DAG_API_PATH}?cwd=${encodeURIComponent('D:\\myprogram\\InsightTutor')}`);
            const res = await handleDagRequest(req, { dbPath: testDbPath });
            expect(res.status).toBe(400);
            const json = await res.json();
            expect(json.ok).toBe(false);
        ...
        it('handleDagRequest returns 404 when dag is not found', async () => {
            const req = new Request(`http://localhost${DAG_API_PATH}?cwd=${encodeURIComponent('D:\\myprogram\\InsightTutor')}&dag_id=unknown-dag`);
            const res = await handleDagRequest(req, { dbPath: testDbPath });
