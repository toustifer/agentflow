...
import { agentsHome, buildMcpConfig, Config, resolveCommand, type AgentflowConfig } from './config.js'
 ...
 * critical capability.
 */
export function apply(ctx: Context, config: AgentflowConfig): void {
  if (config.syncSkill) {
    const target = join(agentsHome(), 'agentflow')
    syncSkill(BUNDLED_SKILL_DIR, target).catch((error: unknown) => {
