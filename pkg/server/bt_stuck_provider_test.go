...
func TestReportStuckOnceReturnsAssignedTaskSnapshot(t *testing.T) {
	...
	require.Equal(t, []string{"task_get"}, resp.SuggestedActions)
...
func TestReportStuckOnceIncludesProjectBlockers(t *testing.T) {
	...
	_, err = eng.CreateNamespace(context.Background(), engine.CreateNamespaceRequest{ID: "ns-1", Name: "test"})
	require.NoError(t, err)
	_, err = eng.CreateTask(context.Background(), engine.CreateTaskRequest{NamespaceID: "ns-1", ID: "T1", Title: "task 1", DependsOn: []string{"T0"}})
	require.NoError(t, err)
	...
	require.Equal(t, []string{"task_get", "project_blockers"}, resp.SuggestedActions)
}
