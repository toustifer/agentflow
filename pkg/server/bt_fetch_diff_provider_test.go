...
func TestFetchWorkDiffOnceReturnsReviewContext(t *testing.T) {
	t.Parallel()
	...
	resp, err := srv.fetchWorkDiffOnce(context.Background(), "ns-1", "T1", "reviewer-a")
	require.NoError(t, err)
	require.Equal(t, "review_pending", resp.State)
	require.NotEmpty(t, resp.Review["commit"])
	require.Contains(t, resp.Prompt, "rev_commit=")
}

func TestFetchWorkDiffProviderHandlesValidation(t *testing.T) {
	t.Parallel()
	...
	srv, err := New(eng, Config{})
	require.NoError(t, err)
	provider, err := newBTFetchWorkDiffProvider(srv)
	require.NoError(t, err)
	defer provider.close()

	req := httptest.NewRequest(http.MethodPost, "/fetch-work-diff", nil)
	req.Header.Set("X-Agentflow-BT-Token", provider.token)
	rec := httptest.NewRecorder()
	provider.handleFetchWorkDiff(rec, req)
	...
	require.NoError(t, err)
	req = httptest.NewRequest(http.MethodPost, "/fetch-work-diff", strings.NewReader(`{"namespace_id":"ns-1","task_id":"missing","worker_id":"reviewer-a"}`))
	req.Header.Set("X-Agentflow-BT-Token", provider.token)
