...
\begin{enumerate}
    \item \textbf{Spatial Confinement}: The Worker agent's tool execution environment is scoped strictly within $\mathcal{P}_i$. File-editing tools reject target paths outside $\mathcal{P}_i$ under the compliant tool-execution path, enforcing task-scoped directory confinement.
...
Crucially, AgentFlow's state persistence is engineered not merely for context compression, but to address \textit{interruption-induced context loss}. When an agent process abruptly aborts due to API timeouts, host restarts, or session budget exhaustion, systems without persisted task-level execution records frequently require agents to reconstruct progress from incomplete scratchpads or replay substantial portions of prior transcripts, multiplying recovery costs and inducing repeated thrashing.
...
\end{equation}
Here, $\oplus$ denotes a keyed composition operator that assembles independently persisted state records into a structured resume package; it does not imply algebraic commutativity or lossless reconstruction of the original conversation. The four constituent dimensions correspond to:
\begin{itemize}
...
\begin{table}[t]
\centering
\caption{Lifecycle Events and Worktree Persistence Semantics}
\label{tab:worktree_lifecycle_semantics}
\begin{tabularx}{\columnwidth}{lXX}
\toprule
\textbf{Lifecycle Event} & \textbf{Worktree Status} & \textbf{Recovery / Transition Action} \\
\midrule
Transient Interruption & Preserved in-place & \texttt{resume} via \texttt{task\_prepare\_start} \\
Lease / Session Expiry & Preserved in-place & Unbind stale agent, re-bind fresh agent \\
Reviewer Rejection & Preserved in-place & Transition to \texttt{rework\_needed}; re-stage diff \\
Task Cancellation & Atomically pruned & Wipe directory; cancel task in FSM \\
Verified Pass & Squash merged & Fast-forward to DAG branch; prune worktree \\
\bottomrule
\end{tabularx}
\end{table}

As formalized in Table~\ref{tab:worktree_lifecycle_semantics}, AgentFlow strictly disambiguates when worktrees are preserved versus destroyed. During transient interruptions, lease timeouts, or reviewer rejections, the worktree directory is deliberately preserved in-place. Upon interruption, a fresh worker agent invokes \texttt{task\_prepare\_start(resume\_targeted=true)}. The engine validates lease freshness, automatically unbinds stale agent sessions, and re-hydrates the agent directly inside the existing worktree with $\mathcal{C}_{\text{resume}}(T_i)$. This avoids full-transcript replay and bounds the injected recovery context by a configurable retrieval budget ($|\mathcal{C}_{\text{resume}}| \leq \mathcal{B}_{\text{retrieval}}$), rather than by the unbounded length of prior conversation. Conversely, atomic worktree pruning is reserved strictly for terminal events: permanent task cancellation or successful post-merge cleanup.
...
\subsection{RQ2: Context Efficiency and Recovery Support}
To evaluate context efficiency and interruption recovery, we measure: (1) prompt token overhead during routine execution, and (2) the recovery efficiency when an in-flight task is abruptly aborted and resumed by a fresh agent session.

\subsubsection{Context Externalization Overhead}
In unpartitioned conversational frameworks (e.g., MetaGPT, conversational SWE-Agent), conversation transcripts grow monotonically. By turn 15--25 of complex multi-file engineering tasks, accumulated input prompts routinely span 4,500--8,500 tokens of raw tool outputs and message history. In contrast, AgentFlow's off-context memory reconstructs targeted operational context on demand:
\begin{itemize}
...
\end{itemize}
When a worker initializes or resumes, it retrieves only task-relevant tuples, yielding a bounded active prompt context $\mathcal{P}_{\text{AgentFlow}} \approx 665$ tokens. Formulating prompt reduction relative to the empirical baseline span ($\mathcal{P}_{\text{Baseline}} \in [4500, 8500]$):
\begin{equation}
\text{Reduction} = 1 - \frac{\mathcal{P}_{\text{AgentFlow}}}{\mathcal{P}_{\text{Baseline}}}
\end{equation}
AgentFlow achieves an estimated \textbf{85.2\%--92.1\% context-budget reduction} in active per-turn prompt overhead relative to the observed baseline transcript span. This substantially reduces exposure to context overflow and limits the injection of unrelated historical debris into each decision step.

\subsubsection{Controlled Interruption Recovery Benchmark}
To empirically test whether persisted operational context ($\mathcal{C}_{\text{resume}}$) genuinely enables task resumption without conversational transcripts, we deployed an automated crash-injection testbed. We interrupted an ongoing implementation task midway (simulating abrupt agent aborts after partial edits) and evaluated four recovery configurations:
\begin{enumerate}
    \item \textbf{AgentFlow (Full)}: Re-hydrated within the preserved worktree using the full four-state operational context ($\mathcal{C}_{\text{resume}}$).
    \item \textbf{Ablation: Git-Only Warm Start}: Re-hydrated within the preserved worktree with physical Git code state, but without FSM lifecycle state, Worker Diaries, or Handbook context.
    \item \textbf{Baseline: Transcript Replay}: Re-hydrated within the preserved worktree accompanied by full historical conversational logs ($>6,000$ tokens).
    \item \textbf{Baseline: Root Cold Start}: A fresh agent initiated in the root repository without the task worktree, execution logs, or diagnostic failure traces.
\end{enumerate}

\begin{table}[t]
\centering
\caption{Empirical Interruption Recovery Performance Across Injected Crash Trials}
\label{tab:interruption_recovery_results}
\begin{tabularx}{\columnwidth}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Resume Success} & \textbf{Recovery Tokens} & \textbf{Resume Latency} & \textbf{Repeated Actions} \\
\midrule
\textbf{AgentFlow (Full)} & \textbf{1/1 (100\%)} & \textbf{720} & \textbf{28.0s} & \textbf{0} \\
Ablation: Git-Only Warm & 1/1 (100\%) & 450 & 25.6s & 0 \\
Baseline: Transcript Replay & 1/1 (100\%) & 6,200 & 24.2s & 0 \\
Baseline: Root Cold Start & 0/1 (0\%) & 380 & 28.0s & N/A \\
\bottomrule
\end{tabularx}
\vspace{0.1cm}
\begin{minipage}{\columnwidth}
\footnotesize
\textit{Note}: Evaluated under an injected mid-implementation abort scenario using identical Gemini 2.5 models and tool budgets. Repeated actions reflect destructive or redundant rollbacks of prior completed work; failed runs are marked N/A.
\end{minipage}
\end{table}

As detailed in Table~\ref{tab:interruption_recovery_results}, the empirical evaluation reveals:
\begin{itemize}
    \item \textbf{Physical Workspace Preservation is the Baseline Prerequisite}: The \textit{Root Cold Start} baseline failed completely (0/1), demonstrating that without spatial worktree preservation or failure contexts, an incoming agent cannot decipher un-merged partial migrations from root trunk files. Conversely, both AgentFlow and \textit{Git-Only Warm Start} succeeded because preserving the physical worktree preserved execution momentum.
    \item \textbf{Replacing Unbounded Transcript Replay}: While \textit{Transcript Replay} succeeded, it demanded \textbf{6,200 prompt tokens} ($8.6\times$ higher than AgentFlow's 720 tokens). AgentFlow achieved identical resumption success while bounding context expenditure to a concise retrieval package.
    \item \textbf{Nuanced Role of Four-State Context}: In this single-module trial, \textit{Git-Only Warm Start} also passed with 450 tokens because the partial implementation in \texttt{auth.py} was sufficiently transparent for the model to deduce the missing function. AgentFlow's 720 tokens provide richer operational context (FSM lease validity, diary handoff, and architectural invariants). The benchmark demonstrates that the structured recovery package matches the success of lighter representations while eliminating transcript replay overhead; it does not by itself establish that all four state dimensions are individually necessary for every simple task.
\end{itemize}
...
\end{enumerate}
In the 18 observed rework events, no rejected diff was merged into the integration branch under the recorded workflow. The rework loop cleanly returned the task to the worker within its isolated worktree, which restaged unpolluted diffs in subsequent iterations.
...
\end{itemize}
Through longitudinal inspection of our event stream, after a pitfall was codified into the Worker Handbook, no recurrence was observed among the instrumented pitfalls in subsequent recorded tasks. Subsequent workers initialized their sessions with direct awareness of these edge cases, mitigating repeated trial-and-error over obscure runtime quirks.
