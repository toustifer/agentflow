...
\title{AgentFlow: A Worktree-Centric, Behavior-Tree-Guided Lifecycle Orchestration Engine for Autonomous Software Teams}
...
\begin{abstract}
Large Language Model (LLM)-based agentic software engineering is undergoing a critical paradigm shift: transitioning from ephemeral, single-issue repairs (e.g., SWE-bench) toward long-horizon, repository-scale feature development. However, existing multi-agent software engineering (SE) frameworks remain brittle when applied to continuous, multi-week codebases. We identify three fundamental structural root causes: (1) \textit{Workspace Contamination}, where erroneous edits by untrusted agents pollute the shared repository, causing unrecoverable baseline damage; (2) \textit{Context Inflation}, where conversational histories grow monotonically, incurring prohibitive token costs and severe context dilution; and (3) \textit{Superficial Delivery Contracts}, where unverified agent self-reports mask build regressions and integration failures.

To address these challenges, we present \textbf{AgentFlow}, a local-first, contract-driven lifecycle orchestration engine for autonomous software teams. AgentFlow decouples rigid state invariants from flexible reactive policies through a novel dual-engine architecture:
(i) a deterministic Go-based finite-state machine (FSM) backed by SQLite, formalizing role-based access control (RBAC) and elevating the Git Worktree into a first-class physical isolation primitive (\textit{One-Task-One-Worktree}) with atomic rollback semantics;
(ii) a reactive Python Behavior Tree (BT) runtime orchestrating the high-level operational lifecycle of Leader, Worker, and Reviewer roles; and
(iii) an off-context, multi-tier memory hierarchy decoupling transient execution logs into structured domain knowledge and architectural pitfall registries.

We empirically evaluate AgentFlow using a mined longitudinal dataset from 19 industrial and research software repositories spanning 82 feature DAGs, 406 tasks, and 1,451 lifecycle events. AgentFlow achieves an effective task completion rate of 78.15\% with a median turnaround duration of 34.8 minutes, yields an 85\%--92\% reduction in active prompt token overhead, enforces a 100\% containment rate of build regressions within isolated worktrees, and reduces repeated framework configuration pitfalls to 0\%.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{Empirical Performance Across 10 Micro-Benchmark Scenarios}
\label{tab:ablation_results}
\begin{tabularx}{\columnwidth}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Pass Rate} & \textbf{Contam.} & \textbf{Tokens/Task} & \textbf{Recovery} \\
\midrule
\textbf{AgentFlow (Full)} & \textbf{100.0\%} & \textbf{0.0\%} & \textbf{2,580} & \textbf{100\% (4/4)} \\
Ablation: No-Worktree & 70.0\% & 40.0\% & 3,060 & 50.0\% (2/4) \\
Ablation: No-Handbook & 80.0\% & \textbf{0.0\%} & 3,630 & 60.0\% (3/5) \\
Baseline: ReAct (SWE) & 40.0\% & 60.0\% & 24,600 & 16.7\% (1/6) \\
\bottomrule
\end{tabularx}
\end{table}
