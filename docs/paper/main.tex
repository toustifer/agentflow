...
\end{equation}
AgentFlow achieves a measured \textbf{85.2\%--92.1\% reduction} in active per-turn prompt overhead.

\subsubsection{Controlled Interruption Recovery Benchmark}
To empirically test whether persisted operational context ($\mathcal{C}_{\text{resume}}$) genuinely enables task resumption without conversational transcripts, we deployed an automated crash-injection testbed. We interrupted an ongoing implementation task midway (simulating host aborts after partial edits) and measured four recovery configurations facing the same interrupted repository snapshot:
\begin{enumerate}
    \item \textbf{AgentFlow (Full)}: Re-hydrated with four-state operational context ($\mathcal{C}_{\text{resume}}$).
    \item \textbf{Ablation: No-State}: Re-hydrated in preserved worktree, but without FSM state or Worker Diary records.
    \item \textbf{Baseline: Transcript Replay}: Replayed raw historical conversation logs ($>6,000$ tokens).
    \item \textbf{Baseline: Git-Only Cold Start}: Fresh agent placed in the root repository without execution logs or initial test errors.
\end{enumerate}
...
\midrule
\textbf{AgentFlow (Full)} & \textbf{100.0\% (Pass)} & \textbf{720} & \textbf{23.0s} & \textbf{0} \\
Ablation: No-State & 100.0\% (Pass) & 450 & 21.9s & 0 \\
Baseline: Transcript Replay & 100.0\% (Pass) & 6,200 & 21.4s & 0 \\
Baseline: Git-Only Cold Start & 0.0\% (Fail) & 380 & 21.7s & 0 \\
\bottomrule
\end{tabularx}
\end{table}

As detailed in Table~\ref{tab:interruption_recovery_results}, the empirical evaluation confirms:
\begin{itemize}
    \item \textbf{Cold-Start Fails without Diagnostic Continuity}: In the \textit{Git-Only} baseline, the incoming agent failed completely because without execution records or failure contexts, it was unaware of prior partial migrations and unpassed assertions.
    \item \textbf{Decoupled Context vs. Transcript Replay}: While \textit{Transcript Replay} succeeded, it incurred \textbf{6,200 recovery prompt tokens} ($8.6\times$ higher than AgentFlow's 720 tokens). AgentFlow achieved identical 100\% pass resumption while keeping context expenditure strictly bounded by the retrieval package.
\end{itemize}
